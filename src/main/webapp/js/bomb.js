 ...
 * 
 */var bombs=[];//炸彈陣列
//var exploreTime=1000;
// 載入炸彈
function loadBomb(fire){

	for(var i=0;i<players.length;i++){
		if(players[i].move.atk == true)
		{
			//set bomb on servlet 
			
			
			players[i].move.atk=false;
			var bomb=models.bomb.mesh.clone();
			var x=UNITWIDTH*parseInt(players[i].mesh.position.x/UNITWIDTH)+2.5+6;
			var z=UNITWIDTH*parseInt(players[i].mesh.position.z/UNITWIDTH)+4;
			bomb.scale.set(2.7,2.7,2.7);
		    bomb.position.set(x,2.5,z);
		    bomb.fire=fire;
		    console.log("bomb.js load bomb x: " +z+" y: "+z);
		     setBomb( x,z,fire); 
		    //animate
		    //bombs.push(bomb);
			//bomb.alive = true;
