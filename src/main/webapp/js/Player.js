...
function rayIntersect(player,ray, distance) {
		...
		for (var i = 0; i < intersects.length; i++) {
				if (player.fire<=5&&intersects[i].distance < UNITWIDTH*1.5) {
						player.fire+=1;
						checkToolFirstMesh(player.mesh.position.clone());
						document.getElementById("fireUp").play();
				}
		}

		intersects = ray.intersectObjects(toolSecondMesh, true);
		for (var i = 0; i < intersects.length; i++) {
				if (player.fire<=3&&intersects[i].distance < UNITWIDTH) {
						player.speed+=0.005;
